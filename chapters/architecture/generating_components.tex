...
To plan the river, we use the same system as used by Teoh \cite{teoh_riverland_2009}. In this approach, we plan a route by placing points in a direction that is randomly offset each point. Between each point we place an additional point such that the river meanders along the route.
...
% Parameters of River
The parameters used to plan the path of the curve are shown in Table \ref{tab:river-planning-paremeters}. The parameters used to create the fat curve are shown in Table \ref{tab:river-fatcurve-parameters}. % Add explanation of param types
...
ControlPoint.position & Vector3 & $\mathbb{R}^3$ & The position of each control point on the curve. \\

ControlPoint.width & float & $> 0$ & The width of the river along each control point. \\
